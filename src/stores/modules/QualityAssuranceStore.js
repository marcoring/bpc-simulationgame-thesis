...
const actions = {
  async getLastVendor({ rootGetters, commit }) {
    ...
    try {
      var response = await axios.get(
        `http://z40lp1.informatik.tu-muenchen.de:8000/sap/opu/odata/sap/Z_40_T2_BIKEGAME_ACF_SRV/QaProcessSet(Guid=guid'${
  ...
  async updateVendors({ commit }) {
    try {
      var response = await axios.get(
        `http://z40lp1.informatik.tu-muenchen.de:8000/sap/opu/odata/sap/Z_40_T2_BIKEGAME_ACF_SRV/ExQaVendorSet?$format=json`
      );
  ...
  async saveVendorIntern({ commit, dispatch, rootGetters }, data) {
    const payload = JSON.stringify({
      ...
      Digitaltwinquality: String(data.Digitaltwinquality),
  ...
  async saveVendorExtern({ commit, dispatch, getters, rootGetters }) {
    const payload = JSON.stringify({
      Guid: rootGetters.gameData.Guid,
      Roundid: rootGetters.gameData.Roundid,
      Userid: rootGetters.gameData.Userid,
