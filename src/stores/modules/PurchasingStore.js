...
const actions = {
  async getLastVendorBattery({ commit, rootGetters }) {
    ...
    try {
      var response = await axios.get(
        `http://z40lp1.informatik.tu-muenchen.de:8000/sap/opu/odata/sap/Z_40_T2_BIKEGAME_ACF_SRV/PurchaseProcessSet(Guid=guid'${
          rootGetters.gameData.Guid
        }',Roundid=${
          rootGetters.gameData.Roundid - 1
        },Materialid='BAT',Userid='${
  ...
  async getLastVendorBatteryM({ getters, commit, rootGetters }) {
    ...
    try {
      ...
      commit(
        "updateLastVendorBatteryM",
        getters.batteryVendors.find(
  ...
  async getLastVendorEngineM({ getters, commit, rootGetters }) {
    ...
    try {
      ...
      commit(
        "updateLastVendorEngineM",
        getters.engineVendors.find(
          (v) => v.Vendorid == response.data.d.Vendorid
        )
      );
    } catch (error) {
    ...
    try {
      var response = await axios.get(
        `http://z40lp1.informatik.tu-muenchen.de:8000/sap/opu/odata/sap/Z_40_T2_BIKEGAME_ACF_SRV/PurchaseProcessSet(Guid=guid'${
          ...
          rootGetters.gameData.Roundid - 1
        },Materialid='FR',Userid='${rootGetters.gameData.Userid}')?$format=json`
      );
  ...
  async getLastVendorFrameM({ getters, commit, rootGetters }) {
    ...
    try {
      ...
      commit(
        "updateLastVendorFrameM",
        getters.frameVendors.find((v) => v.Vendorid == response.data.d.Vendorid)
      );
    } catch (error) {
      if (error.response) {
        // The request was made and the server responded with a status code
  ...
  async getLastVendorSensorsM({ getters, commit, rootGetters }) {
    ...
    try {
      ...
      commit(
        "updateLastVendorSensorsM",
        getters.sensorsVendors.find(
          (v) => v.Vendorid == response.data.d.Vendorid
        )
      );
    } catch (error) {
...
const mutations = {
  updateBatteryVendors: (state, batteryVendors) =>
    (state.batteryVendors = batteryVendors),
  updateBatteryVendor: (state, batteryVendor) =>
    (state.batteryVendor = batteryVendor),
  updateEngineVendors: (state, engineVendors) =>
    (state.engineVendors = engineVendors),
  updateEngineVendor: (state, engineVendor) =>
    (state.engineVendor = engineVendor),
  updateFrameVendors: (state, frameVendors) =>
    (state.frameVendors = frameVendors),
  updateFrameVendor: (state, frameVendor) => (state.frameVendor = frameVendor),
  updateSensorsVendors: (state, sensorsVendors) =>
    (state.sensorsVendors = sensorsVendors),
  ...
  updateLastVendorBattery: (state, lastVendorBattery) =>
    (state.lastVendorBattery = lastVendorBattery),
  updateLastVendorBatteryM: (state, lastVendorBatteryM) =>
    (state.lastVendorBatteryM = lastVendorBatteryM),
  updateLastVendorEngine: (state, lastVendorEngine) =>
    (state.lastVendorEngine = lastVendorEngine),
  updateLastVendorEngineM: (state, lastVendorEngineM) =>
    (state.lastVendorEngineM = lastVendorEngineM),
  updateLastVendorFrame: (state, lastVendorFrame) =>
    (state.lastVendorFrame = lastVendorFrame),
  updateLastVendorFrameM: (state, lastVendorFrameM) =>
    (state.lastVendorFrameM = lastVendorFrameM),
  updateLastVendorSensors: (state, lastVendorSensors) =>
    (state.lastVendorSensors = lastVendorSensors),
  updateLastVendorSensorsM: (state, lastVendorSensorsM) =>
    (state.lastVendorSensorsM = lastVendorSensorsM),
};
